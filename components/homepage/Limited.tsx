...
export default function Limited() {
  ...
  const limitedProducts = products.filter((item) => item.tags?.includes("Limited")).slice(0, 10)
  
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={[styles.title, {color: colorTheme}]}>Limited</Text>
        <TouchableOpacity onPress={() => router.push("/(tabs)/home/limited/")}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => router.push(`/(tabs)/home/limited/${item.id}`)}>
