...
export default function Sale() {
  ...
  const saleProducts = products.filter((item) => item.tags?.includes("Sale")).slice(0, 10)
  
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={[styles.title, {color: colorTheme}]}>Sale</Text>
        <TouchableOpacity onPress={() => router.push("/(tabs)/home/sale/")}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => router.push(`/(tabs)/home/sale/${item.id}`)}>
