...
export default function Popular() {
  ...
  const popularProducts = products.filter((item) => item.tags?.includes("Popular")).slice(0, 10)
  
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={[styles.title, {color: colorTheme}]}>Popular</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity onPress={() => router.push(`/(tabs)/home/popular/${item.id}`)}>
